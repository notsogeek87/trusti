      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="space-y-3">
            ...
            {!isAdding && (
              <button
                ...
                disabled={isSaving}
                className="w-full bg-purple-50 hover:bg-purple-100 text-purple-700 font-semibold py-2.5 px-4 rounded-lg transition-all flex items-center justify-center gap-2 border border-purple-200 disabled:opacity-50 disabled:cursor-not-allowed text-sm"
              >
                <Plus size={18} />
                Ajouter une StarApp
              </button>
            ...
            {isAdding && (
              <div className="bg-white rounded-lg border border-slate-200 overflow-hidden sticky top-0">
                <div className="bg-gradient-to-r from-purple-50 to-pink-50 px-4 py-2.5 border-b border-slate-200">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2 max-h-[calc(95vh-280px)] overflow-y-auto pr-2">{apps.length === 0 ? (
              <div className="text-center py-8">
                <div className="text-4xl mb-2">⭐</div>
                <p className="text-slate-400 font-medium text-sm">Aucune StarApp ajoutée</p>
                <p className="text-xs text-slate-300 mt-1">Cliquez sur "Ajouter une StarApp" pour commencer</p>
              </div>
            ) : (
              apps.map(app => (
                <div key={app.id} className="bg-white border border-slate-200 rounded-lg p-2.5 flex items-center gap-3 hover:border-purple-300 hover:shadow-sm transition-all">
                  {/* Logo */}
                  ...
                  <div className="flex gap-1.5 shrink-0">
                    <button
                      ...
                      disabled={isSaving}
                      className="p-1.5 bg-purple-100 hover:bg-purple-200 text-purple-700 rounded-md transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                      title="Modifier"
                    >
                      <Upload size={14} />
                    </button>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-3xl shadow-2xl max-w-7xl w-full max-h-[95vh] flex flex-col">
        {/* Header */}
        <div className="bg-gradient-to-r from-purple-600 to-pink-600 text-white px-6 py-4 relative shrink-0">
          <button 
            onClick={onClose} 
          ...
          <h2 className="text-xl font-black mb-1">Administration StarApps (Sélection)</h2>
          <p className="text-xs text-white/80">Gérez les applications de la sélection • Interface optimisée pour PC</p>
        </div>
