/**
 * Script d'importation d'applications depuis un CSV vers PostgreSQL/Neon
...
import { getAllApps, createApp } from './service-postgres.js';
...
async function importApps(csvPath) {
  console.log('🚀 Importation d\'applications depuis CSV vers PostgreSQL/Neon\n');
  console.log('━'.repeat(60));
  
  // Récupérer les apps existantes
  console.log('\n📚 Chargement des apps existantes depuis PostgreSQL...');
  ...
  for (let i = 0; i < newApps.length; i++) {
    ...
    try {
      ...
      // Insérer en BDD PostgreSQL
      await createApp(appData);
      successCount++;
      
      console.log(`  ✅ Ajouté avec succès dans PostgreSQL`);
  ...
  console.log(`📚 Total en BDD PostgreSQL: ${existingApps.length + successCount}`);
  console.log('━'.repeat(60));
  console.log('\n✨ Import terminé dans PostgreSQL/Neon !\n');
}
...
if (!fs.existsSync(CSV_PATH)) {
  ...
  console.log('\nUtilisation: node import-csv-apps-postgres.js [chemin-vers-csv]');
  process.exit(1);
}
